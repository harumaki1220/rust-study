fn main() {
    ...
    println!("--------------------------");
    println!("--- 関数と借用 (&) ---");
    let s_len = String::from("this is a test");
    // 
    let len = calculate_length(&s_len);
    // 
    println!("'{}' の長さは {} です。", s_len, len);

    println!("--------------------------");
    println!("--- 関数と可変参照 (&mut) ---");
    let mut s_mut = String::from("hello"); // 
    change_string(&mut s_mut); // 
    println!("変更後のs_mut: {}", s_mut); // 
}
...
fn change_string(s: &mut String) {
    s.push_str(" world"); // .push_str() は文字列に文字を追加する
}